...
import { X, Search, MoreHorizontal, Check, Trash2, ArrowLeft, Plus } from "lucide-react";
...
export default function MultiBadgeSelectPopover({ options, selectedValues, onChange, onUpdateOptions, onClose, disableCreate }: MultiBadgeSelectPopoverProps) {
    ...
    const handleCreateNew = () => {
        const newTerm = `Option ${options.length + 1}`;
        const newOption: BadgeOption = {
            id: `opt-${Date.now()}`,
            value: newTerm,
            label: newTerm,
        ...
        <div ref={popoverRef} className="absolute left-[200px] top-6 z-50 flex animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="bg-[#1E1E1E] border border-gray-700/80 rounded-lg shadow-[0_4px_30px_rgba(0,0,0,0.5)] w-[280px] flex flex-col text-sm text-gray-200 py-2">
                ...
                <div className="flex flex-col py-2 max-h-[300px] overflow-y-auto style-scrollbar">
                    ...
                    {!disableCreate && searchQuery.trim().length === 0 && (
                        <div className="px-2 mt-1 pt-1 border-t border-gray-800">
                            <button
